...
const About = () => {
  ...
  const handleExpand = (index) => {
    setExpandedSpeaker(index === expandedSpeaker ? null : index);
  };
  return (
      ...
      <article className="drive-us d-flex column w--80">
        ...
        <div className="team--div grid a-i-c">
          {Team.map((member, index) => (
            <div key={index} className={`speaker d-flex ${expandedSpeaker === index ? 'expand' : ''} column a-i-c`}>
              ...
              >
                {expandedSpeaker === index ? 'Read less' : 'Know More'}
              </button>
            </div>
          ))}
        </div>
